...
export default function Home() {
  ...
  const [form, setForm] = useState({ marca: "", cidade: "", verba: "", segmento: "", contexto: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.marca.trim() || !form.cidade.trim()) { setErro("Preencha ao menos a marca e a cidade."); return; }
    setLoading(true);
    setErro("");
    setResultado(null);
    setProgresso("Consultando fontes e gerando análise...");
    try {
      const response = await fetch("/api/gerar-pptx", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
        ...
        {!resultado && (
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button type="submit" disabled={loading} className="w-full bg-red-700 hover:bg-red-800 disabled:bg-gray-700 text-white font-semibold py-3 rounded-lg transition-colors flex items-center justify-center gap-2">
                {loading ? (<><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg>{progresso}</>) : "Gerar Análise Scout"}
              </button>
            </form>
          </div>
        )}
        ...
        {resultado && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setResultado(null)} className="text-gray-400 hover:text-white text-sm border border-gray-700 px-4 py-2 rounded-lg transition-colors">Nova consulta</button>
            </div>
